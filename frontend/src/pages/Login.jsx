...
const Login = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      const { role } = await signInWithGoogle(rememberMe);
      navigate('/', { replace: true, state: { roleHint: role } });
    } catch (err) {
      if (err.message === 'ROLE_REQUIRED') {
        setError('Please sign up first to create your account with Google.');
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-2xl w-full max-w-md p-8 relative z-10 border border-white/20">
        ...
        {error && (
          <div className="bg-red-50 border-l-4 border-red-500 text-red-700 px-4 py-3 rounded-lg mb-6 flex items-start space-x-3 animate-in slide-in-from-top">
        ...
        <div className="mt-6">
          ...
          <button
            ...
            disabled={loading}
            className="mt-6 w-full bg-white border-2 border-gray-300 text-gray-700 py-3.5 rounded-xl font-semibold hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm hover:shadow-md flex items-center justify-center space-x-3"
          >
